...
let sortDir = -1;
let currentPortfolioType = ''; // 'custom' or 'accenture401k'
...
portfolioTypeSelect.addEventListener('change', function () {
  ...
  // Auto-load saved portfolios if email already entered
...
async function loadPortfolio(name) {
  ...
  try {
    ...
    if (currentPortfolioType === 'accenture401k') {
      // Repopulate the locked Accenture rows with saved allocations
      buildAccentureFundRows();
      funds.forEach(f => {
